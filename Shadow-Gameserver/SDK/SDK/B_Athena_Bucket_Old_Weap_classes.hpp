#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: B_Athena_Bucket_Old_Weap

#include "Basic.hpp"

#include "B_ConsumableSmall_Athena_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass B_Athena_Bucket_Old_Weap.B_Athena_Bucket_Old_Weap_C
// 0x0000 (0x0B98 - 0x0B98)
class AB_Athena_Bucket_Old_Weap_C final : public AB_ConsumableSmall_Athena_C
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"B_Athena_Bucket_Old_Weap_C">();
	}
	static class AB_Athena_Bucket_Old_Weap_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<AB_Athena_Bucket_Old_Weap_C>();
	}
};
static_assert(alignof(AB_Athena_Bucket_Old_Weap_C) == 0x000008, "Wrong alignment on AB_Athena_Bucket_Old_Weap_C");
static_assert(sizeof(AB_Athena_Bucket_Old_Weap_C) == 0x000B98, "Wrong size on AB_Athena_Bucket_Old_Weap_C");

}

